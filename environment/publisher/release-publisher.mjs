...
async function main() {
  ...
  // --- Import CSV into DuckDB (drop stale tables first) ---
  await exec(conn, `DROP TABLE IF EXISTS raw_manifest`);
  await exec(conn, `DROP TABLE IF EXISTS publishable_builds`);
  await exec(conn, `CREATE TABLE raw_manifest AS SELECT * FROM read_csv_auto('${CSV_PATH}')`);
